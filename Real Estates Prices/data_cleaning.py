...
def draw_heatmap(df, columns, figsize_=8): 
    ...
    sns.heatmap(df.corr(),vmin=-1.0, vmax=1.0, xticklabels=columns, yticklabels=columns)
...
def feature_ordinalEncoding(df, feature_name, inplace=False):
    le = preprocessing.LabelEncoder()
    encoding = le.fit_transform(df[feature_name])
    mapping = dict(zip(le.classes_, le.transform(le.classes_)))

    if inplace:
        df[feature_name] = encoding
        return mapping
    else: 
        df_copy = df.copy()
        df_copy[feature_name] = encoding
        return df, mapping